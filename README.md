...
2. An AI Search instance with indexed data that you want to RAG over. https://learn.microsoft.com/en-us/azure/ai-services/openai/use-your-data-quickstart
3. Access to Azure AI Studio https://ai.azure.com/.

## Setup
Open AI Studio
1. Configure your connections:
    - configured LLM connection with deployed model
2. Open your project, and select *Prompt flow* from the *Tools* menu on the left.
2. On the *Flows* panel, click the *Create* button.
3. Click the *Create* button for the *Chat flow* type. This will open the flow in Prompt flow.
...
There is a default `LLM` node called *chat*. The purpose of an LLM node is to define a prompt, submit it to an LLM, and receive a response. Look at the *Inputs* section at the bottom of the node panel. You can see that it accepts the two parameters passed into the overall flow. The node declares a simple system message, loops through and appends the `chat_history`, and adds the input question `chat_input`. 

## Run the flow
As it is, the flow will run, providing a chat with the configured LLM model.
1. To run the flow you will need to start a compute session. Click the `Start compute session` button at the top of the tool, this may take a minute to start.
...
2. At the top of the default LLM node:
    ...
    - select a model deployment from the deployment_name list, 
    - set max_tokens to 300, 
    - and response_format to *text*.

![alt text](images/llm-config.png)

3. Click the `Chat` button at the top of the screen, a chat panel should open down the right of the page.
...
A Prompt node lets you define a prompt, and create variants of that prompt with the intention of comparing their performance. We are going to create a prompt with some hard coded context data. Later we will replace this context with date from our AI Search service (aka the RAG pattern).
...
2. The following code defines a system message, adds some hard-coded *context* information, and the user question. Paste it into the node.
...
Now we are going to replace the hard-coded data in the Prompt-variants node with data retrieved from AI Search.
...
6. For a bit of clarity, re-order the nodes to reflect the execution order. Using the up arrow in the ribbon at the top of the nodes, shift the *lookup* node under the *chat* node, and the *generate_prompt_context* node under the *lookup* node.
...
2. Click the `Validate and parse input` button below the text area. You should see a additional *contexts* parameter. Set this to be the output of the *generate_prompt_context*, ${generate_prompt_context.output}. Looking at the graph, the final flow should look like the graph below.
